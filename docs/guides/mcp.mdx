...
Handler can expose A2A protocol operations through an MCP server so other tools
can call the same card, message, task, session, and auth operations.
...
Use the MCP server when another tool should drive Handler for you. Use the CLI
when you want direct shell control or automation in scripts.
